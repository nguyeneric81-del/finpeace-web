...
const KR_TRANSLATIONS: Record<string, {
  company_name?: string; sector?: string; strategy_name?: string; timeframe?: string;
  entry_criteria?: string; exit_criteria?: string; analyst_note?: string; catalyst_note?: string; indicators?: string[];
}> = {
  HAH: {
    company_name: "하이안 해운 & 항만 물류 주식회사", sector: "항만 - 물류",
    strategy_name: "대형 기반 돌파 — 상승 추세 지속", timeframe: "중기 (Mid-term)",
  ...
  VOS: {
    company_name: "베트남 해양 운송 주식회사 (VOSCO)", sector: "항만 - 물류",
    strategy_name: "대형 기반 하단 매수 — 반등 포착", timeframe: "중기 (Mid-term)",
  ...
  VHC: {
    company_name: "빙호안 주식회사", sector: "수산 - 식품",
    strategy_name: "달러 강세 수혜 — 수출 마진 확대", timeframe: "중기 (Mid-term)",
  ...
  MWG: {
    company_name: "모바일월드 투자 주식회사", sector: "소매 - 소비재",
    strategy_name: "저점 반등 — 소비 회복 베팅", timeframe: "중기 (Mid-term)",
  ...
  KBC: {
    company_name: "킨박 도시개발 주식회사", sector: "부동산 - 산업단지",
    strategy_name: "FDI 수혜 — 산업단지 실적 급등", timeframe: "중기 (Mid-term)",
  ...
  DGC: {
    company_name: "득장 화학 주식회사", sector: "화학 - 소재",
    strategy_name: "반도체 화학 수요 급증 — 글로벌 테마 수혜", timeframe: "중기 (Mid-term)",
...
const INDICATOR_MAP: Record<string, string> = {
  "Breakout chéo": "대각형 돌파", "Breakout nền": "기반 돌파", "Nến tăng": "양봉",
  "Khối lượng lớn": "대량 거래", "RSI": "RSI", "MACD": "MACD", "EMA": "EMA", "MA": "MA",
  "Hỗ trợ": "지지선", "Kháng cự": "저항선", "Trendline": "추세선", "Sideway": "횡보",
};
...
export default async function TradingPlanKrPage({ params }: { params: Promise<{ ticker: string }> }) {
    ...
    const { data: plan } = await supabase
        .from('trading_plans').select('*').eq('ticker', ticker).single();
