...
function ContentBlockRenderer({ block }: { block: ContentBlock }) {
    switch (block.type) {
        ...
        case 'quote':
            return (
                <div className="bg-slate-800 rounded-2xl p-6 my-2">
                    ...
                    <p className="text-white text-lg font-medium italic leading-relaxed mb-4">{block.content as string}</p>
                    ...
                    <div className="space-y-2.5">
                        {(block.content as string[]).map((item, i) => (
                            <div key={i} className="flex items-start gap-3">
                                <div className="w-5 h-5 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-700 font-bold text-xs shrink-0 mt-0.5">{i + 1}</div>
        ...
        case 'steps':
            return (
                <div>
                    {block.title && <h3 className="text-slate-800 font-bold text-lg mb-5">{block.title}</h3>}
                    <div className="space-y-4">
                        {(block.items ?? []).map((item: ContentBlockItem, i: number) => (
                            <div key={i} className="flex gap-4 p-4 bg-white border border-slate-100 rounded-2xl hover:shadow-sm transition-shadow">
                                <div className="flex flex-col items-center gap-1 shrink-0">
                                    <div className="w-8 h-8 bg-emerald-600 text-white rounded-xl flex items-center justify-center font-black text-sm">{i + 1}</div>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex flex-wrap items-center gap-2 mb-1.5">
                                        ...
                                        {item.highlight && (
                                            <span className="text-xs font-black text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full">{item.highlight}</span>
                                        )}
                                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {(block.patterns ?? []).map((p: CandlePatternItem, i: number) => (
                            <div key={i} className="bg-white border border-slate-100 rounded-2xl p-3 flex flex-col items-center gap-2 hover:shadow-sm transition-shadow">
                                <div className="flex items-end justify-center h-[90px] gap-1"><CandleChart candles={p.candles} /></div>
                                <p className="text-slate-800 font-bold text-xs text-center leading-tight">{p.name}</p>
                                <span className={`text-[10px] font-black px-2 py-0.5 rounded-full border ${p.signal === 'bullish' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : p.signal === 'bearish' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                            ...
                            <div key={i} className="px-6 py-4 hover:bg-slate-50 transition-colors">
                                <div className="flex items-start gap-3">
                                    <div className="shrink-0 min-w-[72px]">
                                        <span className="inline-block bg-slate-800 text-white text-[10px] font-black px-2 py-0.5 rounded-md">{clause.number}</span>
                                    </div>
            ...
            <section className={`${pillar.color}`}>
                <div className="max-w-4xl mx-auto px-6 py-12">
                    <motion.div {...fadeUp(0)} className="flex items-center gap-2 mb-4 flex-wrap">
                        <span className={`text-xs font-bold px-3 py-1 rounded-full border ${DIFFICULTY_COLOR[article.difficulty]}`}>{article.difficulty}</span>
                        <span className="flex items-center gap-1 text-xs text-slate-400"><Clock className="w-3 h-3" /> {article.readTime} phút đọc</span>
                        <span className="flex items-center gap-1.5 text-xs text-slate-400">
                            <span className="text-base">{pillar.icon}</span>
                            <span className={pillar.accentColor + ' font-semibold'}>{pillar.title}</span>
                        </span>
                    </motion.div>
                    <motion.h1 {...fadeUp(0.08)} className="text-3xl lg:text-4xl font-black text-slate-800 leading-tight mb-4">{article.title}</motion.h1>
                    <motion.p {...fadeUp(0.12)} className="text-slate-500 text-base leading-relaxed mb-6 max-w-2xl">{article.summary}</motion.p>
                    ...
                    <div className="grid lg:grid-cols-3 gap-12">
                        ...
                        <div className="space-y-5">
                            <motion.div {...fadeUp(0.1)} className={`${pillar.color} border ${pillar.borderColor} rounded-2xl p-5`}>
                                <p className={`text-xs font-bold uppercase tracking-widest ${pillar.accentColor} mb-3`}>Tiến Độ Trong Chủ Đề</p>
                                <div className="space-y-2">
                                    {pillar.articles.map((a, i) => (
                                        <Link key={a.slug} href={`/knowledgebase/${pillar.slug}/${a.slug}`}
                                            className={`flex items-center gap-2 text-xs rounded-xl px-3 py-2 transition-colors ${a.slug === articleSlug ? `bg-white ${pillar.accentColor} font-bold shadow-sm` : 'text-slate-500 hover:bg-white/60'}`}>
                                            <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-black shrink-0 ${a.slug === articleSlug ? pillar.color + ' ' + pillar.accentColor : 'bg-slate-100 text-slate-400'}`}>{i + 1}</span>
                            ...
                            <motion.div {...fadeUp(0.15)} className="bg-slate-50 border border-slate-200 rounded-2xl p-5">
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between text-sm"><span className="text-slate-500">Thời gian đọc</span><span className="font-semibold text-slate-700">{article.readTime} phút</span></div>
                                    <div className="flex justify-between text-sm"><span className="text-slate-500">Độ khó</span><span className={`font-semibold text-xs px-2 py-0.5 rounded-full ${DIFFICULTY_COLOR[article.difficulty]}`}>{article.difficulty}</span></div>
                                    <div className="flex justify-between text-sm"><span className="text-slate-500">Chủ đề</span><span className="font-semibold text-slate-700">{pillar.title}</span></div>
                                </div>
                            </motion.div>
                        </div>
                    </div>
            ...
            <section className="bg-neutral-50">
                <div className="max-w-4xl mx-auto px-6 py-8">
                    <div className="grid grid-cols-2 gap-4">
                        {prevArticle ? (
                            <motion.div {...fadeUp()}>
                                <Link href={`/knowledgebase/${pillar.slug}/${prevArticle.slug}`} className="flex-1 bg-white border border-slate-200 rounded-2xl p-4 hover:shadow-md transition-shadow group block">
                                    <div className="flex items-center gap-2 text-slate-400 text-xs mb-1"><ChevronLeft className="w-3 h-3" /> Bài trước</div>
                                    <p className="text-slate-700 font-semibold text-sm leading-snug group-hover:text-emerald-700 transition-colors line-clamp-2">{prevArticle.title}</p>
                                </Link>
                            </motion.div>
                        ) : <div />}
                        {nextArticle ? (
                            <motion.div {...fadeUp(0.05)}>
                                <Link href={`/knowledgebase/${pillar.slug}/${nextArticle.slug}`} className="flex-1 bg-white border border-slate-200 rounded-2xl p-4 hover:shadow-md transition-shadow group text-right block">
                                    <div className="flex items-center justify-end gap-2 text-slate-400 text-xs mb-1">Bài tiếp <ChevronRight className="w-3 h-3" /></div>
                                    <p className="text-slate-700 font-semibold text-sm leading-snug group-hover:text-emerald-700 transition-colors line-clamp-2">{nextArticle.title}</p>
                                </Link>
                            </motion.div>
                        ) : <div />}
                    </div>
                    <motion.div {...fadeUp(0.1)} className="mt-4 text-center">
                        <Link href={`/knowledgebase/${pillar.slug}`} className={`inline-flex items-center gap-2 text-sm font-semibold ${pillar.accentColor} hover:underline`}>
