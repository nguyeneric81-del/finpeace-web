...
interface MacroStory {
  ...
  accent_color: string
  behind_story: { point: string; quote: string; source: string }[]
...
interface Props {
  agent: Agent
  story: MacroStory     // currently only macro, future: union type
  lpConfig: any
  agentCode: string
  topicSlug: string
  lpId: string | null
  contentType: string   // 'macro_insight' | 'product' | 'event' (future)
}

export default function SalesLandingPageClient({ agent, story, lpConfig, agentCode, topicSlug, lpId, contentType }: Props) {
  ...
  const accentColor = story.accent_color || accent
  ...
  return (
    <div className="min-h-screen text-slate-100" style={{ background: primary, fontFamily: "'Be Vietnam Pro', system-ui, sans-serif" }}>
      ...
      {/* ── AGENT HEADER ── */}
      <header className="px-6 py-5 flex items-center gap-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        {agent.avatar_url ? (
        ...
        <div>
          <p className="text-xs font-bold uppercase tracking-widest" style={{ opacity: 0.5 }}>{agent.brand_name}</p>
          <p className="font-bold text-white">{agent.full_name}</p>
          {agent.title && <p className="text-xs" style={{ opacity: 0.4 }}>{agent.title}</p>}
        </div>
        <div className="ml-auto flex items-center gap-3 flex-wrap">
          {agent.contact_phone && (
            <a href={`tel:${agent.contact_phone}`} className="flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-full" style={{ background: accent + '20', color: accent }}>
              <Phone className="w-3.5 h-3.5" />{agent.contact_phone}
            </a>
          )}
          {agent.contact_zalo && (
            <a href={`https://zalo.me/${agent.contact_zalo}`} target="_blank" className="flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-full" style={{ background: '#0068FF25', color: '#60A5FA' }}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12 space-y-14">
        ...
        {/* ── HERO ── */}
        <section>
          <span className="inline-block px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-6" style={{ background: accentColor + '20', color: accentColor, border: `1px solid ${accentColor}40` }}>
          ...
          {story.impact_value && (
            <div className="flex items-start gap-3 rounded-2xl p-5" style={{
              background: story.impact_positive ? 'rgba(16,185,129,0.1)' : 'rgba(244,63,94,0.1)',
              border: `1px solid ${story.impact_positive ? 'rgba(16,185,129,0.25)' : 'rgba(244,63,94,0.25)'}`,
            }}>
        ...
        {story.key_stats?.length > 0 && (
          <section>
            <p className="text-xs font-bold uppercase tracking-widest mb-5 flex items-center gap-2" style={{ opacity: 0.45 }}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {story.key_stats.map((s, i) => (
                <div key={i} className="rounded-2xl p-5 space-y-2" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
                  <p className="text-2xl font-black" style={{ color: s.positive === true ? '#34d399' : s.positive === false ? '#fb7185' : accentColor, fontFamily: 'monospace' }}>{s.value}</p>
                  <p className="text-xs font-medium leading-snug" style={{ opacity: 0.55 }}>{s.label}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        {story.data_point && (
          <section className="rounded-2xl p-6" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ opacity: 0.45 }}>Dữ liệu thực tế</p>
            <p className="text-base font-semibold" style={{ color: accentColor }}>{story.data_point}</p>
          </section>
        )}
        ...
        {story.behind_story?.length > 0 && (
          <section>
            <h2 className="text-xl font-bold text-white mb-8">Câu chuyện đằng sau</h2>
            <div className="relative">
              <div className="absolute left-5 top-0 bottom-0 w-px" style={{ background: 'rgba(255,255,255,0.08)' }} />
              <div className="space-y-8">
                {story.behind_story.map((item, i) => (
                  <div key={i} className="relative pl-14">
                    <div className="absolute left-0 w-10 h-10 rounded-full flex items-center justify-center text-sm font-black border" style={{ background: primary, borderColor: accentColor + '50', color: accentColor }}>{i + 1}</div>
                    <p className="text-base font-semibold text-white mb-3">{item.point}</p>
                    <div className="rounded-xl p-4 text-sm italic" style={{ background: 'rgba(255,255,255,0.03)', borderLeft: `3px solid ${accentColor}50`, color: 'rgba(255,255,255,0.55)' }}>
                      &ldquo;{item.quote}&rdquo;
                      <p className="text-xs font-bold mt-2 not-italic" style={{ color: accentColor }}>{item.source}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {story.analyst_view && (
          <section className="rounded-2xl p-7" style={{ background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)' }}>
            ...
            <p className="font-medium leading-relaxed" style={{ color: 'rgba(255,235,180,0.8)' }}>&ldquo;{story.analyst_view}&rdquo;</p>
          </section>
        )}
        ...
        <section className="rounded-3xl p-8 md:p-10" style={{ background: 'rgba(255,255,255,0.05)', border: `1px solid ${accent}40` }}>
          {submitted ? (
            ...
            <>
              ...
              {agent.brand_tagline && <p className="text-sm mb-8" style={{ color: 'rgba(255,255,255,0.4)' }}>{agent.brand_tagline}</p>}
        ...
        <footer className="text-center text-xs pb-8" style={{ color: 'rgba(255,255,255,0.15)' }}>
