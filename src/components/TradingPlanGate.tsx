...
const LABELS = {
  vi: {
    ...
    btn: 'Xem Kế hoạch Ngay', success: 'Mở khóa thành công!', success_sub: 'Đang tải kế hoạch...',
    disclaimer: 'Thông tin chỉ dùng để liên hệ tư vấn. Không spam.',
    entry_zone: '📥 Vùng Mua', stop_loss: '🛑 Cắt Lỗ', take_profit: '🎯 Chốt Lời', rr: '⚡ Tỷ lệ R:R',
    chart_label: '📊 Biểu đồ Phân tích Kỹ thuật',
    entry_title: 'Điều kiện vào lệnh', exit_title: 'Kế hoạch thoát lệnh', analyst_title: 'Nhận định từ chuyên gia',
    catalyst_prefix: '💡 Catalyst Tác động:', indicators_label: 'Tín hiệu Technical sử dụng',
    err_email: 'Vui lòng nhập email hợp lệ', err_phone: 'Vui lòng nhập số điện thoại',
    err_server: 'Có lỗi xảy ra, vui lòng thử lại', loading: 'Đang xử lý...',
  },
  kr: {
    strategy: '전략',
    unlock_title: '트레이딩 플랜을 보려면 등록하세요',
    unlock_sub: 'FinPeace Advisor 팀의 상세 트레이딩 플랜을 받으려면 정보를 입력하세요',
    name: '성함', email: '이메일 *', phone: '전화번호 *',
    btn: '지금 플랜 보기', success: '인증 완료!', success_sub: '플랜을 불러오는 중...',
    disclaimer: '정보는 상담 연락 목적으로만 사용됩니다. 스팸 없음.',
    entry_zone: '📥 매수 구간', stop_loss: '🛑 손절', take_profit: '🎯 익절', rr: '⚡ R:R 비율',
    chart_label: '📊 기술적 분석 차트',
    entry_title: '진입 조건', exit_title: '청산 계획', analyst_title: '전문가 의견',
    catalyst_prefix: '💡 촉매 요인:', indicators_label: '사용된 기술적 신호',
    err_email: '유효한 이메일을 입력하세요', err_phone: '전화번호를 입력하세요',
    err_server: '오류가 발생했습니다. 다시 시도해 주세요', loading: '처리 중...',
  }
}

export default function TradingPlanGate({
  plan, lang = 'vi',
  strategy_name, timeframe, entry_criteria, exit_criteria, analyst_note, catalyst_note, indicators
}: Props) {
  const t = LABELS[lang]
  ...
  useEffect(() => { if (isTpUnlocked()) setUnlocked(true) }, [])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/knowledgebase/leads', {
        ...
        body: JSON.stringify({ name, email, phone, sales_code: getSalesCode(), source: `trading-plan-${plan.ticker}`, pillar: 'trading-plan', article_slug: plan.ticker }),
      })
  ...
  return (
    <>
      ...
      {/* Gate: form (normal flow) OR full content */}
      {unlocked ? (
        <div className="space-y-6">
          {entryCriteria && (
            <div className="flex gap-4">
              <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center shrink-0 mt-1"><TrendingUp className="w-5 h-5 text-blue-600" /></div>
              <div><h3 className="font-bold text-slate-800 text-lg mb-2">{t.entry_title}</h3><p className="text-slate-600 leading-relaxed">{entryCriteria}</p></div>
            </div>
          )}
          {exitCriteria && (
            <div className="flex gap-4">
              <div className="w-10 h-10 rounded-full bg-rose-100 flex items-center justify-center shrink-0 mt-1"><ShieldAlert className="w-5 h-5 text-rose-600" /></div>
              <div><h3 className="font-bold text-slate-800 text-lg mb-2">{t.exit_title}</h3><p className="text-slate-600 leading-relaxed">{exitCriteria}</p></div>
            </div>
          )}
          {analystNote && (
            <div className="flex gap-4">
              <div className="w-10 h-10 rounded-full bg-amber-100 flex items-center justify-center shrink-0 mt-1"><CheckCircle2 className="w-5 h-5 text-amber-600" /></div>
              <div>
                ...
                <div className="bg-amber-50/50 border border-amber-100 p-4 rounded-xl">
                  ...
                  {catalystNote && <p className="mt-4 pt-4 border-t border-amber-200/50 text-sm text-amber-800"><strong>{t.catalyst_prefix}</strong> {catalystNote}</p>}
                </div>
              </div>
            </div>
          )}
          {inds.length > 0 && (
            <div className="mt-8 pt-6 border-t border-slate-100">
              ...
              <div className="flex flex-wrap gap-2">
                {inds.map((ind, idx) => <span key={idx} className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full text-xs font-medium">{ind}</span>)}
              </div>
            </div>
          )}
        </div>
      ) : (
        /* ✅ Normal flow — không dùng absolute, button không bị clip */
        <div className="w-full max-w-sm mx-auto mt-2 pb-4">
          {success ? (
