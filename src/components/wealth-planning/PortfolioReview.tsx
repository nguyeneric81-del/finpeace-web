...
import {
    ...
    Layers, ArrowRight, CheckCircle2, ChevronRight, Sparkles, Zap
} from "lucide-react"
    ...
    return (
        ...
        >
            ...
            <div className="w-full h-1 rounded-full bg-white/10 mb-3">
                <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: status === "good" ? "85%" : status === "warn" ? "50%" : "20%" }}
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <section>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-800/80 to-slate-900/80 border border-white/10 backdrop-blur p-6 shadow-xl">
                        ...
                        <div className="flex flex-col items-center gap-1.5 w-full">
                            <motion.div initial={{ opacity: 0, scaleX: 0.6 }} animate={{ opacity: 1, scaleX: 1 }} transition={{ delay: 0.4 }}
                                ...
                                <div className="absolute opacity-0 group-hover:opacity-100 bg-black/90 text-white text-xs p-2 rounded -top-10 whitespace-nowrap z-10 transition-opacity">
                    ...
                    <div className="rounded-2xl bg-gradient-to-br from-slate-800/80 to-slate-900/80 border border-white/10 backdrop-blur p-6 space-y-4 overflow-y-auto max-h-[480px] shadow-xl">
                        ...
                        {assets.length === 0 && (
                            <p className="text-sm text-white/30 text-center py-8">Chưa có tài sản nào được khai báo</p>
                        )}
                    </div>
                </div>
            </section>
                    ...
                    <div className="rounded-2xl bg-gradient-to-br from-slate-800/80 to-slate-900/80 border border-white/10 backdrop-blur p-6 space-y-6 shadow-xl">
                        ...
                        <div className="pt-4 border-t border-white/10">
                            <div className="flex justify-between items-center mb-3">
                                <div>
                                    <p className="text-xs font-bold text-white/80 flex items-center gap-1.5">
                                        ...
                                        Đồng Hồ PYF — Trả Cho Mình Trước
                                    </p>
                                    <p className="text-[10px] text-white/30 mt-0.5">Tỷ lệ Thu Nhập đang được Gieo Hạt mỗi năm</p>
                                </div>
                            ...
                            <div className="relative h-3 bg-white/10 rounded-full overflow-hidden">
                                ...
                                />
                                <div className="absolute top-0 bottom-0 border-l border-white/30 border-dashed" style={{ left: "20%" }} />
                            </div>
                            <div className="flex justify-between text-[10px] text-white/30 mt-1.5">
                                <span>0%</span>
                                <span className="text-emerald-500 font-medium">↑ Mục tiêu 20%</span>
                                <span>100%</span>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-3 pt-2 border-t border-white/10">
                            <div className="flex-1 bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3">
                                <p className="text-[10px] text-emerald-400 font-semibold mb-1 flex items-center gap-1"><TrendingUp className="w-3 h-3" />Tiết Kiệm / Năm</p>
                                <p className="text-sm font-bold text-white/90">{fmtVND(cashflow.annual_saving)}</p>
                            </div>
                            <div className="flex-1 bg-sky-500/10 border border-sky-500/20 rounded-xl p-3">
                                <p className="text-[10px] text-sky-400 font-semibold mb-1 flex items-center gap-1"><Droplets className="w-3 h-3" />Dòng Tiền Còn Lại / Năm</p>
                                <p className="text-sm font-bold text-white/90">{fmtVND(cashflow.annual_income - cashflow.annual_expense)}</p>
                            </div>
                        </div>
                    </div>
                </section>
            ...
            <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.5 }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {computed.recommendations.map((rec, i) => (
                        <motion.div
                            ...
                            className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${rec.color} border backdrop-blur p-5 shadow-lg hover:-translate-y-1 hover:shadow-xl transition-all duration-300 cursor-default group`}
                        >
                            <div className="absolute top-0 right-0 w-16 h-16 rounded-full bg-white/5 -mr-4 -mt-4" />
                            <div className="flex items-start justify-between mb-3">
                ...
                {/* Philosophy */}
                <div className="mt-6 rounded-2xl bg-gradient-to-r from-emerald-500/10 via-teal-500/5 to-violet-500/10 border border-white/10 p-5">
                    <p className="text-xs text-white/60 leading-relaxed italic flex gap-2">
                        <Sparkles className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                        <span><strong className="text-white/80">Nhắc nhở từ Bình An Tài Chính:</strong> Bạn không cần đợi đến khi có 5 tỷ mới hạnh phúc.
                        Ngay khoảnh khắc bạn biết mình đang đứng ở đâu, và chọn một hành động nhỏ hôm nay — bạn đã bước vào Vùng Đất Bình An rồi.</span>
                    </p>
                </div>
            </motion.section>
        </div>
